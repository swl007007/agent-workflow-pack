...
def test_final_wheel_contains_frozen_provenance_licenses_and_notices(
    ...
    with zipfile.ZipFile(release_artifact_set.wheel.path) as archive:
        provenance = json.loads(archive.read("agent_stack/data/release/provenance-lock.json"))
        notices = archive.read("agent_stack/data/THIRD_PARTY_NOTICES.md")
        pyyaml_license = archive.read("agent_stack/data/LICENSES/PyYAML-6.0.2.txt")
        fast_license = archive.read("agent_stack/data/LICENSES/fastjsonschema-2.21.1.txt")

    assert provenance["provenance_lock_digest"] == release_artifact_set.provenance_lock_digest
    assert notices
