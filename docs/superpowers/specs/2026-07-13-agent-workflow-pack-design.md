...
The scanner requires verified discovery evidence for both sides. The snapshot has digest domain `agent-workflow.task-quiescence.v1` and records the source and target layout digests and schema-bundle digests; every normalized current task path and its source/target active-or-archive discovery roles; stable task identity; integration byte hash, normalized mode, schema ID/version, lifecycle status, state revision, and `task_contract_digest`; every metadata path, byte hash, mode, parser/classifier identity, parsed task references, semantic role, and empty/nonempty classification; and every task-transaction path, byte hash, mode, schema, operation, phase, task ref, and terminal classification. It also records sorted finding objects for unknown entries, collisions, limit violations, interpretation conflicts, unfinished journals, non-archived tasks, and layout-preservation failures. All sets are sorted by normalized repository-relative path and stable IDs before JCS.

Task identity and task-contract identity are normative rather than adapter-defined:

```text
task_identity
  = normalize_repo_relative(integration.admission.task_ref)
...
`admission.task_ref` is the immutable original active task ref recorded at admission and remains unchanged when the task moves to an archive path; the snapshot records the current discovered path separately. `normalized_workflow_contract` is exactly the closed `workflow_contract` object in Section 21 after schema validation and normalization, with no diagnostic or derived digest field. Duplicate `task_identity` values, a non-normalizable admission ref, or a stored/current ref inconsistency is an interpretation-conflict finding.

The scanner enumerates the union of source and target active/archive partitions, metadata contracts, and task-transaction roots while applying each side's own verified schemas. A source-only path is interpreted only by the verified source declarations; a target-only path only by the verified target declarations; an overlapping path is parsed independently under both. A path visible under both contracts must have the same task identity, active/archive role, lifecycle classification, `task_contract_digest`, metadata semantic role and task references, and journal operation, phase, task ref, and terminal result. Conflicts become `layout-ambiguous` findings; valid nonterminal journals become `unfinished-task-transaction` findings; non-archived integrations become `non-archived-task` findings. The scanner does not convert any finding into a command error code.
...
`evaluate_task_gate` is the only layer that maps those facts to operation-specific blockers. `candidate_impact` is a versioned closed object derived by the Resolver from the candidate profile/lock/artifact/policy/adapter diff; it is never caller-supplied free text. At minimum it distinguishes `none`, general contract change, Trellis layout/runtime, route admission, platform adapters, hooks, agents, related skills, and other runtime-visible file changes. The normalized impact, evaluator ID/version, findings, and resulting blockers are plan-bound. Section 14.12 defines the strict workspace-migration policy; Section 17 defines lifecycle policy.
...
The dimensions are independent, and `primary_blocker` is null only when the requested command has no blocking condition. A target-owned source-to-target edge may establish `relationship: migration-required` with `relationship_evidence: verified` while `discovery_evidence: missing`; task quiescence is then `not-evaluated`. `ahead` and `diverged` require verified relationship evidence but do not require the current runtime to understand the source Trellis parser/classifier. Discovery `unsupported` or authentic-but-invalid sets task quiescence to `ambiguous` only when a command needs task discovery; it does not erase an already verified release relationship. A completed scan plus `evaluate_task_gate` yields `quiescent` when there are no operation-specific blockers and `blocked` otherwise.

Initial workspace-state primary selection is deterministic and conditional. Any cryptographic manifest, asset, identity, or bundle-root mismatch is `AWP_SOURCE_RELEASE_VERIFICATION_FAILED` with exit category 30 before ordinary workspace blocker ordering. Missing relationship evidence yields `AWP_WORKSPACE_SOURCE_METADATA_REQUIRED`; verified `ahead` or `diverged` then yields its relationship blocker regardless of discovery support. For `migration-required`, missing discovery evidence yields `AWP_WORKSPACE_SOURCE_METADATA_REQUIRED`, unsupported/invalid discovery or layout-ambiguous findings yield `AWP_WORKSPACE_TASK_LAYOUT_AMBIGUOUS`, and the command-specific evaluator then selects task-recovery, active-task, or stranded-layout blockers. `AWP_WORKSPACE_MIGRATION_REQUIRED` is primary only when relationship and required discovery evidence are verified and no more specific evaluator blocker exists. Migration-required state permits read-only diagnostics, `workspace migrate`, and only recovery already authorized by the current runtime allowlist. Ahead or diverged state permits read-only diagnostics and independently authorized recovery but not migration. All routing, task mutation, provider execution, and Reconciler-backed lifecycle writes remain blocked until the contracts match. An old task journal whose pinned runtime is no longer the committed or lifecycle-candidate runtime is reported as a recovery blocker; project-local target state cannot authorize execution of that source runtime.

`AWP_TASK_QUIESCENCE_CHANGED` is not part of this initial-state ordering. It is a transaction stale-evidence error: after a plan or workspace-migration journal binds a snapshot, any later rescan mismatch makes `AWP_TASK_QUIESCENCE_CHANGED` the command's primary error unconditionally. Findings from the latest scan, including a newly created active task, are retained only as secondary diagnostics until the transaction is replanned or safely recovered.
...
Ordinary `doctor` is strictly read-only. It checks schemas, digests, cache, external runtimes, capabilities, ownership, drift, routing graph, workspace-contract alignment, unfinished transactions, active-task compatibility, source/target task-discovery evidence, static mount facts, and any previously recorded filesystem-probe evidence. For a differing registered local contract it emits the Section 12 structured relationship-evidence/discovery-evidence/task-quiescence diagnostic and uses the shared initial-state priority. Missing cached detached-manifest or compatibility bytes are `relationship_evidence: missing` only when they are required to classify the graph; missing layout/integration/metadata/journal bytes are independently `discovery_evidence: missing`. Unsupported parser/classifier IDs set discovery to `unsupported` and quiescence to `ambiguous` without changing a verified relationship. Any cached manifest, asset, identity, or bundle-root mismatch reports `AWP_SOURCE_RELEASE_VERIFICATION_FAILED` in exit category 30. `doctor` never fetches, creates locks, refreshes evidence, intentionally updates target or cache state, or treats partial transaction state as success. A filesystem property that cannot be established from still-valid evidence is reported as `unverified`, not guessed as passing.
...
`workspace migrate` then calls `evaluate_task_gate(operation: workspace-migrate, candidate_impact: local-state-contract-change, findings)`. This strict policy maps every layout-ambiguous finding to `AWP_WORKSPACE_TASK_LAYOUT_AMBIGUOUS`, every unfinished task transaction to `AWP_WORKSPACE_TASK_RECOVERY_BLOCK`, every non-archived task to `AWP_WORKSPACE_ACTIVE_TASK_BLOCK`, and every valid stranded-state finding to `AWP_WORKSPACE_LAYOUT_STATE_STRANDED`. Diagnostics identify the source release and instruct the user to restore that project checkout, run the matching task recovery when necessary, complete the task, and archive it before pulling/retrying the target migration. Archiving is necessary but is not sufficient when the target layout would not recognize that archived state; v0.1 requires the source-only partition or metadata state to be absent/canonical-empty and provides no task-layout mover. The pulled target Manifest, launcher, or compatibility edge cannot authorize the source runtime. v0.1 has no retained catalog or loader exception: `workspace migrate` never edits, resumes, upgrades, archives, reindexes, or reclassifies task state.
...
Planning records the canonical snapshot, findings, digest, normalized candidate impact, and evaluator result in `plan_core`. The scanner does not decide whether a discovered task blocks the command. `evaluate_task_gate` applies these lifecycle rules:
...
- For `upgrade`, every non-archived `speckit-superpowers` task blocks a contract-changing candidate. A non-archived `trellis-native` task blocks only when normalized candidate impact changes Trellis layout/runtime, route admission, its platform adapter, hooks, agents, related skills, or another surface pinned by its `task_contract_digest`.
- `sync` and `repair` use the same mode/impact comparison for runtime-visible writes. A task finding that consumes none of the changed surfaces is not itself a blocker.
...
- A true no-op `sync` has `candidate_impact: none`, may return verified no-op even when non-archived findings exist, and performs no write transaction. Findings remain available as diagnostics rather than being mislabeled as scanner errors.
...
`task_state_digest` covers the canonical non-archived-task scan, normalized stable `task_identity` values, active pointers, modes, lifecycle revisions, and recomputed `task_contract_digest` values. The integrated branch also covers the requested task-ref absence precondition. It is null only for a classification-only project with no task-state inputs. Staging under `.agent-workflow/local/` does not alter this digest. Any intervening admission, transition, archive, pointer change, contract change, or relevant ref creation makes an executable Decision stale.
...
`admission.task_ref` is immutable and is the task's stable identity source even after archive moves the integration file. Its canonical task identity is the normalized repository-relative admission ref; the current active/archive file location is separate scanner evidence. Any mode-specific task-ref field must equal this canonical identity. The workflow contract is a closed object, and its derived identity is:
...
`normalized_workflow_contract` contains exactly the schema-versioned fields shown above after path/string/version normalization and excludes diagnostics and the derived digest itself. Loaders, task-state digests, scanners, and candidate-impact comparison recompute this value rather than accepting a caller-supplied contract identity.
...
clone C: pull a target with no source-to-target edge
  ...
  -> launcher, doctor, and migrate emit identical relationship/relationship-evidence/discovery-evidence/quiescence dimensions and primary blocker
...
- **AC-12:** The shared scanner reports task facts without choosing command policy. `evaluate_task_gate` makes every non-archived heavy task block a contract-changing upgrade, while a non-archived Trellis-native task blocks only when normalized candidate impact intersects its pinned Trellis, routing, adapter, hook, agent, related-skill, or other task-contract surface.
...
- **AC-57:** Launcher, `doctor`, and `workspace migrate` emit one structured workspace-state diagnostic with independent `relationship`, `relationship_evidence`, `discovery_evidence`, and `task_quiescence` dimensions plus a deterministic initial-state `primary_blocker`. Unsupported discovery may make quiescence ambiguous without changing verified ahead/diverged; missing bytes report `AWP_WORKSPACE_SOURCE_METADATA_REQUIRED`, while manifest/hash/identity/bundle-root mismatch reports `AWP_SOURCE_RELEASE_VERIFICATION_FAILED` with exit 30.
- **AC-58:** Stable task identity is the normalized immutable `integration.admission.task_ref`, distinct from the current active/archive path. `task_contract_digest = SHA256(UTF8("agent-workflow.task-contract.v1\0") || UTF8(JCS(normalized_workflow_contract)))`; scanners, Route Decision state, loaders, and candidate-impact evaluation recompute the same domain-separated value and reject duplicate identities or inconsistent refs.
...
4. **Runtime launcher and Task-state Service** — single-file launcher cold bootstrap, split pre-start container/post-start package verification, two-stage caller context, post-wheel descriptor validation, pinned runtime delivery, atomic workspace/ledger registration, source-static-metadata verification, the shared bounded Trellis quiescence scanner and versioned operation-specific task-gate evaluator, strict no-active-task/no-stranded-state cross-clone workspace migration, versioned replay ledger and outbox, integration union, task locks and CAS, claims, admitting/archive transactions, recovery, and maintenance coordination.
...
The eleventh-version architecture remains stable, but this candidate is still `Changes required` until the scanner/evaluator separation, independent relationship/discovery evidence, transaction-stale error priority, and normative task identity/contract digest receive focused reviewer approval. Implementation planning and feature-spec decomposition remain prohibited until that confirmation.
