...
| Artifact-bundle runtime-surface registry and unit inventory | Stable surface IDs, canonical ownership of every runtime-visible rendered or packaged unit, digest recipes, transitive references, full-coverage proof, and authority/surface impact projection |
...
evaluate_workspace_state_quiescence(
  ...
  findings
) -> task_quiescence + state blockers
...
The scanner never applies command policy. `evaluate_workspace_state_quiescence` is the fixed, command-independent state evaluator used by the launcher, `doctor`, and every workspace diagnostic. It applies the strict workspace-migration/state policy: any discovery or interpretation ambiguity yields `ambiguous`; any unfinished task transaction, non-archived task, or valid stranded-layout finding yields `blocked`; and only a fully interpreted snapshot with none of those findings yields `quiescent`. The requested command, candidate impact, and no-op exception never enter this evaluator. `evaluate_task_gate` separately maps the same facts to operation-specific command blockers and consumes the complete snapshot because findings alone do not contain enough mode or contract detail to evaluate impact.

The Resolver derives `candidate_impact` from the current contract, observed materialized state, and candidate IR; it is never caller-supplied free text:

```yaml
schema_version: 1
impact_kind: none # or runtime-visible
authority_changes:
  - authority_id: release-identity
    before_digest: sha256-old
    after_digest: sha256-new
surface_changes:
  - surface_id: platform-adapter:codex
    change_kind: contract-change
    contract_before_digest: sha256-old
    observed_before_digest: sha256-old
    after_digest: sha256-new
  - surface_id: skill:tdd
    change_kind: contract-change
    contract_before_digest: sha256-value
    observed_before_digest: sha256-value
    after_digest: canonical-null
```

A restorative repair uses the same closed schema but cannot coexist with an authority or contract change:
...
surface_changes:
  - surface_id: runtime-entry:trellis-implement
    change_kind: repair
    contract_before_digest: sha256-expected
    observed_before_digest: canonical-null
    after_digest: sha256-expected
```

Surface IDs come from a closed, versioned registry in the artifact bundle. Reserved namespaces cover `runtime-control-plane`, `surface-registry`, `trellis-runtime`, `trellis-layout`, `route-policy`, `router:<id>`, `platform-adapter:<platform>`, `hook:<platform>:<id>`, `agent:<platform>:<id>`, `skill:<id>`, and `runtime-entry:<id>`; unknown namespaces or free-form selectors are schema errors. Every integrated task surface closure includes `runtime-control-plane` and `surface-registry` plus its mode-, platform-, route-, and entry-specific transitive references.

The registry is accompanied by a closed runtime-visible-unit inventory. Every managed render unit, generated instruction block, wrapper or loader, hook, agent, skill, command projection, adapter, policy/router artifact, runtime entry, and first-party module importable from a supported CLI entry in the self-contained wheel has exactly one canonical owning surface and may influence other surfaces only through the declared acyclic reference graph. Build/test sources absent from every distribution are outside this inventory; packaged helper modules are not. A unit without an owner, a duplicate or ambiguous owner, a dangling reference, an uncovered runtime entry, an inventory byte not represented in its owning digest recipe, or a runtime-visible candidate byte absent from the inventory blocks Resolver output and release. Registry schema, digest-recipe, inventory, or reference-graph changes are owned by `surface-registry`; packaged CLI modules default to `runtime-control-plane` unless the closed registry assigns a narrower canonical owner. Release CI recomputes coverage from wheel, sdist, Git-checkout package, and rendered bytes rather than trusting registry claims.

Each `surface_digest` is domain-separated and binds the surface descriptor, digest-recipe version, sorted owned-unit byte/contract digests, and sorted referenced surface IDs plus their digests. Registry source objects never contain computed unit or surface digests. The `surface-registry` digest uses a closed projection of registry schema, source descriptors, identity/path/kind/owner inventory, digest recipes, and reference edges that excludes derived unit/surface roots and the generated coverage proof; ordinary content change in an otherwise unchanged surface therefore does not make every task appear registry-affected. Other surfaces may reference the registry digest, but the acyclic graph forbids a reverse dependency. The generated coverage proof remains artifact-bundle/release evidence, not a runtime-surface input or independent authority. `canonical-null` represents absence. `authority_changes` is the complete sorted before/after vector for `release-identity`, `profile`, `workflow-lock`, `artifact-bundle`, `route-policy`, `router-contract`, `surface-registry`, and `trellis-layout`; unchanged entries are omitted, while an omitted change or inconsistent digest invalidates the impact object. A candidate is `contract-changing` for a heavy task when this authority vector is nonempty or any `surface_change` has `change_kind: contract-change`. Every release upgrade therefore changes at least `release-identity`, even if selected task surface bytes happen to match. A restorative repair has an empty authority vector.

`surface_changes` is the complete sorted union of candidate contract changes and observed drift selected for explicit repair. `contract_before_digest` is the current registry's expected digest, `observed_before_digest` is the digest of the actual materialized surface or `canonical-null`, and `after_digest` is the candidate contract digest. `contract-change` requires `after_digest != contract_before_digest`; ordinary sync/upgrade also requires observed state to equal the current contract or it blocks for explicit repair. `repair` requires `contract_before_digest == after_digest`, a differing observation, an unchanged registry/reference graph/authority vector, explicit approval, and byte-and-mode CAS. A task's `task_contract_surfaces` is the complete exact set it may load or execute. The evaluator matches stable IDs and verifies the task's pinned digest equals `contract_before_digest`; that contract mismatch is stale/ambiguous evidence. An observation mismatch is instead valid repair evidence only under `change_kind: repair` and never authorizes a different after digest.

Multiple command blockers use one deterministic order: layout/discovery ambiguity, unfinished task transaction, affected non-archived task, then stranded layout state; ties sort by canonical task ID, normalized current path, surface ID, authority ID, and finding ID. Workspace-state blockers use the same fact ordering but are produced only by the fixed state evaluator. The normalized snapshot, impact object, both evaluator IDs/versions and results, full blocker lists, and selected primary evaluator blocker are plan-bound. Section 14.12 defines the strict workspace-migration command policy; Section 17 defines lifecycle policy.
...
- the closed runtime-surface registry, runtime-visible-unit inventory, each surface's canonical digest recipe, coverage proof, and transitive-reference graph;
...
The coverage proof is a release-neutral canonical witness over the enumerated unit IDs, normalized package/render paths, byte or contract leaf digests, canonical owning surfaces, and reference validation result. It contains no `artifact_bundle_digest`, distribution hash, detached-manifest digest, or other value derived from the Merkle root that includes it; release dependency-graph tests reject any such reverse edge.
...
- the closed runtime-surface registry, unit inventory, reference closure, and verified full-coverage result;
- render units and artifact definitions;
- normalized authority-change and surface-change impact, including restorative-repair observations when applicable;
...
A target-owned source-to-target edge may establish `relationship: migration-required` with `relationship_evidence: verified` while `discovery_evidence: missing`; task quiescence is then `not-evaluated`. `ahead` and `diverged` require verified relationship evidence but do not require the current runtime to understand the source Trellis parser/classifier. Discovery `unsupported` or authentic-but-invalid sets task quiescence to `ambiguous` only when task discovery is required; it does not erase an already verified release relationship. After a completed scan, only the versioned `evaluate_workspace_state_quiescence(snapshot, findings)` result populates `workspace_state.task_quiescence`. It is identical for the launcher, `doctor`, `workspace migrate`, and every other command over the same verified bytes: ambiguity yields `ambiguous`, unfinished/non-archived/stranded state yields `blocked`, and otherwise the result is `quiescent`. Operation-specific `evaluate_task_gate` results affect only `command_admission` and never rewrite this state dimension.
...
Creates an explicit restorative-repair plan for missing or drifted pack-managed content. It never silently overwrites a divergent preimage. For every runtime-visible surface, the plan separately records the current contract digest, observed preimage digest or `canonical-null`, and candidate after digest. A repair is restorative only when `contract_before_digest == after_digest`, the current Manifest/registry still authorizes that contract, the authority-change vector is empty, and the registry/inventory/reference graph are unchanged. The observed digest may differ because the managed bytes or mode are missing or drifted, but it remains a CAS precondition rather than being substituted for the task's pinned contract digest.

The plan must show the expected baseline, actual state, candidate bytes, normalized repair impact, and active-task evaluation and must be approved like any other write transaction. For a task that pins the repaired surface, repair is allowed only when the after digest exactly equals both its pinned surface digest and the current contract digest. A different after digest is a contract change, not repair, and follows the normal active-task blocker. Runtime loading remains fail-closed while a required surface is drifted; after a committed restorative repair it can resume without changing `integration.yaml`, the task revision, or its pinned surface set.
...
Runs deterministic policy, graph, runtime-surface inventory/full-coverage, impact-normalization, golden-case, and rendered-adapter checks. It accepts normalized signal IDs rather than interpreting natural language.
...
### 14.10 `task runtime load|admit|claim|transition|release|archive|recover`

`agent-stack task runtime load` is the canonical read-only admission and dispatch path for an already admitted integrated task. It accepts the normalized current task ref, immutable task ID, expected state revision, expected lifecycle phase and executor-claim state, requested `surface_id`, and requested runtime-entry ID. It does not accept a Route Decision, approval proof, caller-supplied digest, arbitrary filesystem path, or free-form mode override. The requested surface must be the registry-declared canonical owner of that runtime entry, normally `runtime-entry:<id>`. While holding the runtime-state gate for the authorization snapshot, the pinned CLI rereads and validates `integration.yaml`, recomputes `task_contract_digest`, verifies task ID/ref and expected revision/phase/claim, verifies the entry contract authorizes the current pinned mode, phase, owner, and claim state, and verifies the requested owning surface and every referenced dependency are in the closed pinned set. It reads all declared bytes needed by that dispatch through no-follow regular-file/package-resource access into an immutable in-memory dispatch bundle, then recomputes for each required surface the current contract digest from the verified registry and the observed digest from those bytes, modes, and contract metadata using the same recipe; it requires `observed_surface_digest == contract_surface_digest == pinned_surface_digest`. The selected adapter dispatches only from that validated bundle and may not reopen a managed runtime-catalog path. Authorization is not emitted as a reusable bearer token. A state or byte change detected before bundle completion fails closed and requires a fresh call.

The original `create-integrated-task` Decision and approval remain immutable admission provenance after revision 1 commits. Their identity and recorded bindings remain schema-valid, but their ref-absence, task-state, revision, and freshness preconditions are never re-evaluated for existing-task resume. Unrelated aggregate profile, lock, artifact-bundle, policy, or router digest changes do not by themselves authorize or deny a Trellis-native runtime load; the current project authorities must be valid, and the exact requested surface membership and digest are the runtime criterion. Heavy upgrades cannot create such aggregate divergence because Section 17 blocks their authority-change vector while the task is non-archived.

The remaining commands are the only supported mutation interface for integrated task lifecycle. `admit` consumes only an `operation: create-integrated-task` Decision and its one-time user-approval proof, then creates the task shell and revision 1 together through the task-admission transaction in Section 21. `classify-only` and `execute-light` are invalid inputs. `claim`, `transition`, and `release` require the expected revision plus command-specific preconditions. `archive` coordinates the locked Trellis archive adapter and integration lifecycle in one recoverable task transaction. `task recover --transaction <id> --resume|--rollback` may resume any validated unfinished task transaction, but rollback is legal only before that operation's commit point; after commit it may finish forward completion or cleanup and may not erase the committed task or archive. Platform wrappers must not expose direct Trellis create/archive, direct integration writes, arbitrary runtime-catalog paths, or non-interactive approval bypasses.
...
`plan_core` contains all normalized operation inputs, identities, authorities, preconditions, non-Manifest candidate file states, the candidate local-state contract derived from release/schema inputs, provider approvals, the applicable Section 9 `task_quiescence_snapshot`, findings, `task_quiescence_digest`, normalized `candidate_impact`, fixed workspace-state evaluator ID/version/result, operation-specific task-gate evaluator ID/version/result, prospective transaction ID, and recovery runtime. A plan eligible for approval has an empty command blocker set even when its scanner findings or command-independent workspace-state blockers are nonempty, as may occur for a true no-op `sync`. It excludes `plan_core_digest`, `journal_binding_digest`, candidate Manifest bytes/digest, `plan_digest`, and presentation diagnostics. The immutable header contains the fields defined in Section 16 but uses `plan_core_digest`, never final `plan_digest`, and copies the task-quiescence digest as an explicit sibling binding. The plan envelope contains `plan_core`, `plan_core_digest`, the immutable header, `journal_binding_digest`, `candidate_manifest_digest`, and the candidate Manifest file-state record. Candidate Manifest bytes are reconstructable from the locked inputs and must match both their file-state record and `candidate_manifest_digest`; any workspace candidate must independently render the identical `plan_core` local-state contract rather than reading candidate Manifest bytes. Serialization may flatten schema fields, but the digest domains and exclusions are normative.
...
plan_core:
  ...
  candidate_impact:
    ...
    authority_changes:
      - authority_id: release-identity
        before_digest: sha256-old-release
        after_digest: sha256-new-release
    surface_changes:
      - surface_id: platform-adapter:codex
        change_kind: contract-change
        contract_before_digest: sha256-old
...
Planning records the canonical snapshot, findings, digest, normalized candidate impact, fixed workspace-state evaluation, and command-specific task-gate evaluation in `plan_core`. The scanner does not decide whether a discovered task blocks the command, and the fixed evaluator does not receive the requested operation. `evaluate_task_gate(operation, candidate_impact, task_quiescence_snapshot, findings)` applies these lifecycle rules only to command admission:
...
- For `upgrade`, every non-archived `speckit-superpowers` task blocks a contract-changing candidate. `contract-changing` is the closed Section 9 predicate: a nonempty authority-change vector or any `surface_change` with `change_kind: contract-change`. A release upgrade is therefore heavy-contract-changing even when the task-specific surface bytes match. A non-archived `trellis-native` task blocks only when a contract-changing `surface_change` matches one of its exact pinned `task_contract_surfaces` and changes or removes that surface's digest. Adapter, hook, agent, skill, router, registry, control-plane, and runtime-entry impact therefore comes from stable surface intersection rather than inference from an aggregate task-contract digest.
- A normal runtime-visible `sync` uses the same surface comparison and cannot absorb observed drift. A task finding whose pinned surface set has no contract-changing ID is not itself a blocker; a pinned digest that differs from `contract_before_digest` is stale/ambiguous evidence and fails closed.
- `repair` uses the explicit restorative branch. A matching task surface does not block when `change_kind: repair`, `contract_before_digest == after_digest == pinned surface_digest`, the authority vector is empty, and the registry/inventory/reference graph are unchanged. `observed_before_digest` may be different or `canonical-null` and is enforced by byte-and-mode CAS. Any repair record with a different after digest, a contract mismatch, an authority/registry change, or an unclassified runtime-visible byte is invalid or becomes a normal contract-changing blocker. Runtime load stays blocked until the restorative transaction commits, then resumes against the same pinned contract.
...
- A true no-op `sync` has `impact_kind: none`, empty authority and surface change lists, and no observed drift selected for repair. It may return verified no-op even when non-archived findings exist and performs no write transaction. Findings remain available as diagnostics rather than being mislabeled as scanner errors.
...
The Router consumes the same compiled admission policy as the adapter. Before task admission it may revalidate Decision identity, matched rule IDs, signal IDs, task state, pinned digests, and approval, but may not maintain a second independent signal list. After admission it receives runtime content only through `task runtime load` and validates current integration phase/claim plus pinned surfaces rather than replaying the stale create Decision.
...
Route Decisions are not signed and have no issuer-authenticity guarantee. The calculator, UUID, and digest prove only canonical internal consistency and freshness for the supplied fields. A policy-consistent envelope constructed elsewhere is indistinguishable from calculator output and is not treated as trusted because of its claimed origin. The executable Decision consumers therefore reread current authorities, recompute identities, rerun the compiled policy over the supplied stable signal IDs, validate the operation/route union, and require exact agreement on route, matched rules, entry owner, task state, adapter version, and approval requirement. Those consumers are the `execute-light` loader before native-light execution and `task admit` before revision 1 commits. Modified, inconsistent, stale, or cross-operation envelopes fail; origin alone is never an authorization input.

After integrated admission commits, the Decision's task-ref-absence and task-state preconditions are necessarily stale and are not a runtime-resume credential. Existing-task execution uses only the Section 14.10/20 task-runtime load contract: current integration identity and lifecycle preconditions plus exact pinned surface membership and digest. The recorded Decision and approval remain immutable provenance and are checked for internal schema/digest consistency when the integration is validated, but they are not policy-replayed or freshness-checked on each task load.
...
The wrapper has two closed execution branches. `execute-light` validates and consumes a fresh `operation: execute-light` Decision before entering only the native-light binding. An already admitted integrated task never reuses its create Decision; the wrapper invokes `agent-stack task runtime load` with the task ID/ref, expected lifecycle revision/phase/claim, surface ID, and runtime-entry ID and lets the pinned CLI validate and dispatch the exact entry without returning a reusable authorization token.

Before loading an integrated runtime entry, that pinned CLI validates:

- maintenance state;
- the committed runtime, Manifest, workspace contract, runtime-surface registry, inventory, coverage proof, and reference graph;
- integration schema, immutable task ID, current normalized task location, recomputed task-contract digest, and the recorded admission provenance's internal consistency;
- expected state revision, lifecycle phase, pinned mode, mode-specific entry owner, executor claim, and the runtime entry's closed allowed-mode/phase/claim predicate;
- requested `surface_id` membership in `workflow_contract.task_contract_surfaces`;
- current contract and observed surface digest equality with the task's pinned surface digest for the owning surface and every transitive dependency, including `runtime-control-plane` and `surface-registry`;
- requested runtime-entry ownership by that surface and its exact locked content digest and POSIX mode.

The aggregate profile, lock, artifact-bundle, policy, adapter, and router fields recorded at admission are provenance inputs to `task_contract_digest`; they are not individually compared to current aggregate values as a substitute for surface authorization. A missing, drifted, unclassified, newly referenced, or digest-mismatched unit fails closed. Restorative repair is the only path that may replace an observed drifted surface with the same pinned/current contract digest; until that transaction commits, runtime load remains blocked.
...
`workflow_contract.task_contract_surfaces` is a closed, sorted set of `{surface_id, surface_digest}` records derived by the route calculator and reverified by `task admit`; models and adapters cannot append free-form surfaces. It contains every exact runtime surface the task may consume, including transitive references and the mandatory `runtime-control-plane` and `surface-registry` meta-surfaces. Its digest must match the Route Decision's `task_contract_surfaces_digest`. After admission, this set plus current lifecycle state is runtime authorization authority; the recorded Decision is provenance rather than a freshness credential. The workflow contract is a closed object, and its derived identity is:
...
`normalized_workflow_contract` contains exactly the schema-versioned fields shown above after path/string/version/surface normalization and excludes diagnostics and the derived digest itself. Task-runtime loaders, task-state digests, scanners, and candidate-impact comparison recompute this value rather than accepting a caller-supplied contract identity. Surface records sort by `surface_id`; duplicates, unknown IDs, missing mandatory/meta/transitive references, or a surface digest inconsistent with the admission authorities invalidate the integration contract. Runtime load additionally requires the requested current surface digest to equal this pinned value; aggregate admission digests cannot replace that check.
...
- existing-task resume after its create Decision becomes stale, using integration revision/phase/claim and exact pinned surface authorization;
- integrated runtime-load rejection for an unpinned entry, wrong task ID/ref, stale revision/claim, missing mandatory meta-surface, or current surface/content digest mismatch;
- unrelated Trellis-native surface changes remaining loadable while a consumed adapter, skill, control-plane, registry, or runtime-entry change blocks;
...
- the recorded create Decision remaining immutable provenance rather than being accepted as an existing-task runtime credential.
...
Test duplicate YAML keys, inheritance cycles, unknown fields, JCS digests, set sorting, Merkle inputs, path normalization, POSIX-mode normalization, marker parsing, Release Identity, detached-manifest trust rules, release-compatibility edges, source static-metadata schemas, the closed Trellis task-discovery/layout union, segment and filename grammars, integration/metadata/journal parser-classifier identities, UUID task identity, historical task-ref reuse, task-surface registry/inventory IDs and digests, surface digest recipes, acyclic transitive references, full runtime-visible-unit coverage, `task_contract_digest`, task-runtime load authorization inputs, task-quiescence snapshots/findings/digests, scanner-versus-fixed-state-evaluator-versus-command-gate separation, authority-change vectors, contract-change versus restorative-repair surface impact normalization, the structured workspace-state/command-admission diagnostic, the saved-plan operation union, the complete acyclic `plan_core_digest -> journal_binding_digest -> candidate_manifest_digest -> plan_digest` graph, closed Route Decision branches, task/provider approval-verifier unions, caller-context envelopes, workspace-registration and workspace-migration journals, workspace/replay/outbox schema migrations, replay state transitions, provider-attempt journals and release receipts, and Manifest generations. Coverage tests reject an unowned or multiply owned runtime-visible unit, a dangling or cyclic surface reference, an omitted package control-plane module, a digest recipe that omits owned bytes, a runtime entry outside the inventory, and registry/reference changes absent from `surface-registry` impact. Dependency-graph tests also reject computed unit/surface roots entering registry source, a generated coverage proof entering `surface-registry`, an artifact-bundle digest entering its coverage proof, a final `plan_digest` entering `journal_binding_digest` or candidate Manifest input, and a workspace candidate deriving its contract from candidate Manifest bytes instead of the shared `plan_core` contract. Property/fuzz tests target path normalization, bounded metadata expansion, task-segment grammar, maximum depth/count/byte bounds, unknown-root entries, source/target task-root union and partitioning, surface-ID grammar and ordering, runtime-load surface/entry selectors, archive extraction, release-manifest URLs and redirects, general URL handling, caller-context field normalization, and marker parsers.

### 26.2 Resolver and Policy Tests

Test dependencies, conflicts, stable IDs, capability ordering, route rules, source-of-truth separation, Trellis runtime-namespace constraints, closed discovery/parser/classifier references, metadata-path cross-ownership rejection against every protected or artifact-managed path class, the fixed command-independent workspace-state evaluator, and every `evaluate_task_gate` operation/mode/snapshot/authority/surface-impact combination. Golden policy cases include strict workspace migration; heavy authority-vector contract change even with byte-identical task surfaces; Codex-adapter affected versus Claude-adapter unaffected Trellis-native tasks; `skill:tdd` affected versus unrelated-skill unaffected tasks; control-plane, registry, hook, agent, router, and runtime-entry changes; removed surfaces; pinned-versus-contract digest mismatch; deterministic multi-blocker ordering; missing or drifted consumed surfaces represented as restorative `sync --repair`; allowed `observed != contract_before == after == pinned`; rejected repair-to-different-contract and repair-with-authority-change cases; and true no-op `sync` with non-archived findings.

### 26.3 Golden Rendering

Snapshot Claude Code, Codex, and OpenCode output. Verify route-gated runtime content is absent from auto-discovery paths. Re-enumerate every rendered runtime-visible unit, prove exact registry ownership and digest inclusion, and compare the resulting coverage root with the packaged artifact bundle. Execute each locked initializer twice in independent isolated roots and compare both runs with its lock-bound content-root digest.

### 26.4 Reconciler Tests

Use temporary projects to test every ownership class, protected paths, symlink refusal, byte-and-mode CAS, executable-bit changes, restorative repair, repair-to-different-contract rejection, overlay retirement, single-file launcher authority, post-wheel descriptor validation, mixed launcher/descriptor preimage and candidate states, cold-cache launcher-bound bootstrap, existing-task runtime load through pinned integration/surface authorization, stale create-Decision non-reuse, unpinned-entry and surface/content-drift rejection, successful resume after a consumed adapter/runtime entry is restored to its pinned digest, transaction-created directory cleanup, atomic workspace-plus-ledger registration, lifecycle local-state migration, independent post-pull workspace migration, trusted static inspection of a source release without code execution, unfinished and corrupt/unknown-schema source task journals, strict workspace-migration non-archived blocking, mode/surface-sensitive lifecycle gating, true no-op sync with non-archived findings, unknown task-root files/directories, invalid task segments, excess scan depth/count/size, metadata pointer/index schema disagreement, source-only archived tasks, source-only nonempty metadata, target-only task state, layout changes whose one-sided state is entirely absent/canonical-empty, same-clone current/candidate layout changes, verified `ahead` with unsupported discovery parser, invalid authenticated relationship schema with exit 30, separate workspace-state and per-command admission diagnostics, command-independent quiescence equality across launcher/`doctor`/migration, supply-chain mismatch exit-30 behavior, deterministic state/evaluator blocker priority, every saved-plan operation and digest branch, cross-clone saved-plan refusal, saved-plan staleness, read-only `doctor`, explicit write-probe cleanup and interruption, filesystem capability refusal, and no-write conflict behavior.
...
Runtime-load race tests change integration revision/phase/claim, replace or remove the requested runtime bytes, and alter the registry/reference graph at every boundary between authorization and dispatch. Because the pinned CLI constructs and dispatches one immutable validated in-memory bundle rather than returning a bearer token or reopening catalog paths, each race either dispatches the fully validated preimage under the held snapshot or fails before runtime content executes; it never loads an unpinned or partially repaired entry.
...
clone A: init
  ...
  -> resume the admitted task through task runtime load without reusing the now-stale create Decision
  -> reject an unpinned runtime entry and a mismatched revision/claim
  -> remove a pinned adapter/runtime-entry file and assert runtime load fails closed
  -> approve restorative sync --repair with observed null and after digest equal to the pinned/current contract
  -> commit repair under CAS and resume the same task without changing its revision or surface set
  -> drift conflict
  -> assert zero writes
  ...
  -> mark the replacement task completed
...
- the packaged and rendered runtime-visible-unit inventories have complete, unambiguous surface ownership; control-plane, registry, digest-recipe, and reference-graph mutations always produce normalized surface/authority impact and no unclassified runtime byte can reach release;
...
- an admitted task resumes through pinned integration/surface authorization after its create Decision becomes stale, rejects unpinned or drifted entries, and resumes after an approved CAS-protected restorative repair returns the observed surface to the same pinned/current contract digest;
...
- **AC-07:** Missing or drifted managed content blocks normal sync and runtime loading when consumed. It can be restored only through an approved, CAS-protected `sync --repair` whose contract-before and after digests equal the current registry contract; observed drift is recorded separately and never becomes authority for different bytes.
...
- **AC-12:** The shared scanner reports task facts without choosing command policy. `evaluate_task_gate` consumes operation, normalized candidate authority/surface impact, the complete task snapshot, and findings. Every non-archived heavy task blocks a contract-changing upgrade, where contract-changing is a nonempty authority vector or contract-changing surface record. A non-archived Trellis-native task blocks only when a stable contract-changing surface ID and contract-before digest match one of its exact pinned Trellis, routing, control-plane, registry, adapter, hook, agent, skill, or runtime-entry surfaces.
...
- **AC-25:** Every `create-integrated-task` Decision and approval proof binds one random canonical task ID, task ref, task-surface digest, intent, operation, workspace, and one-time challenge. The canonical calculator generates the ID, while `task admit` enforces binding, freshness, and uniqueness without claiming unsigned-decision issuer authenticity. The Decision is consumed only for creation and becomes provenance after revision 1; the task shell and integration commit through the specified admission transaction, and every admission or archive crash point is diagnosable, recoverable under CAS, and blocked from becoming a partially accepted task state.
...
- **AC-55:** Same-clone lifecycle commands and post-pull `workspace migrate` invoke the same `scan_task_quiescence` implementation over verified source/target layouts and schemas. It returns only canonical snapshot/findings across active/archive partitions, metadata objects, and task journals. The fixed versioned workspace-state evaluator is command-independent, while a separate `evaluate_task_gate(operation, candidate_impact, snapshot, findings)` applies strict workspace-migration command policy, mode/surface-sensitive lifecycle policy, deterministic multi-blocker ordering, restorative-repair semantics, and the zero-write no-op sync exception; current-only scanners or command-specific fallback parsers cannot satisfy any layer.
...
- **AC-59:** `task_contract_surfaces` and `candidate_impact.surface_changes` use one closed registry of stable surface IDs and complete domain-separated digests. Adapter-specific and skill-specific affected cases block, unrelated surfaces do not, removed surfaces use canonical null, pinned-versus-contract-before mismatch fails closed, observed drift is represented only by the repair branch, and the full state/gate evaluator inputs and results are plan-bound.
...
- **AC-61:** Existing integrated-task runtime loading never requires the stale create Decision. `agent-stack task runtime load` validates current integration, task ID/ref, expected revision/phase/claim, the entry's allowed mode/phase/claim predicate, requested pinned surface membership, equality of observed/current-contract/pinned digests for the owning surface and every dependency, runtime-entry ownership/content/mode, mandatory control-plane and registry dependencies, and maintenance in one pinned CLI dispatch path. It rejects arbitrary paths, reusable authorization tokens, unpinned or drifted entries, and state changes before dispatch; the recorded Decision remains provenance only.
- **AC-62:** The runtime-surface registry has a release-verified full-coverage invariant. Every runtime-visible rendered or packaged unit has one canonical owning surface and complete digest inclusion; unowned, multiply owned, dangling, cyclic, omitted, or unclassified units block Resolver/release. Registry, inventory, digest-recipe, reference-graph, and relevant CLI control-plane changes appear through the mandatory `surface-registry` or `runtime-control-plane` surfaces and normalized authority/surface impact. Heavy `contract-changing` is exactly a nonempty authority vector or contract-changing surface record, so every release upgrade blocks a non-archived heavy task.
- **AC-63:** Restorative repair records `contract_before_digest`, `observed_before_digest`, and `after_digest` separately. A consumed drifted surface may be repaired during a non-archived task only when the authority vector is empty and `contract_before_digest == after_digest == task pinned surface_digest`; byte-and-mode CAS protects the observation. Repair to any other digest, repair with registry/authority change, or ordinary sync/upgrade over unexplained drift blocks. Runtime load fails before repair commit and succeeds afterward without mutating the task contract or revision.
- **AC-64:** `workspace_state.task_quiescence` is produced only by the fixed command-independent state evaluator: ambiguity yields `ambiguous`, unfinished/non-archived/stranded state yields `blocked`, and otherwise it yields `quiescent`. Requested operation, candidate impact, no-op policy, and restorative-repair eligibility affect only `command_admission`; launcher, `doctor`, migration, sync, repair, and upgrade cannot report different workspace task-quiescence for identical verified evidence.
...
3. **Renderer and Reconciler** — staging, ownership, operation-discriminated plans, OS lock, CAS, transactions, restorative repair with separate contract/observation digests, and recovery.
4. **Runtime launcher and Task-state Service** — single-file launcher cold bootstrap, split pre-start container/post-start package verification, two-stage caller context, post-wheel descriptor validation, pinned runtime delivery and existing-task runtime-load authorization/dispatch, atomic workspace/ledger registration, source-static-metadata verification, split workspace-state/command-admission diagnostics, the shared bounded Trellis quiescence scanner and fixed-state/snapshot-surface-aware command evaluators, strict no-active-task/no-stranded-state cross-clone workspace migration, versioned replay ledger and outbox, UUID task identity and safe ref reuse, integration union, task locks and CAS, claims, admitting/archive transactions, recovery, and maintenance coordination.
5. **Route admission and Platform Adapters** — compiled creation/light admission policy, closed Route Decision union, intent-bound executable signals, approval-verifier envelopes, generated wrappers over the runtime-load service, runtime catalog, Trellis root/metadata cross-ownership contracts, capability probes, adapter projection, and platform golden output.
