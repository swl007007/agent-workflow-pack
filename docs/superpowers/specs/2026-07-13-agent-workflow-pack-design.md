...
| `.agent-workflow/local/workspace.json` | Non-Git identity of the current working copy plus the last locally applied release and aggregate workspace/replay/outbox contract |
...
transitions:
  - from: 0.1.0
    ...
    resume_compatibility:
      - source_task_contract_id: sha256-source-task-contract
        ...
        retained_content:
          adapter: sha256-value
          heavy-development-router: sha256-value
          speckit-runtime: sha256-value
    migration_digest: sha256-value
```

Edges are directed; reverse support requires its own entry. Each edge binds the target logical Release Identity and expected trust-policy, workflow-lock, artifact, schema, migration, and launcher bundles, but never a source commit, its own compatibility-bundle digest, or a distribution-container URL or hash; `target_release.version` must equal `to`. It also declares the from/to aggregate local-state contract digests and schema versions for every persistent project and workspace state domain used by the transition. A changed workspace, replay-ledger, or task-outbox schema requires an explicit migration ID and digest; an unfinished task transaction pins its original schema versions and blocks an upgrade whose edge cannot continue or migrate them. No transition may reset or silently recreate local state to avoid a migration. The detached manifest independently binds the source commit and the digest of the compatibility bundle containing the edge. An edge never implies compatibility with an unlisted patch, minor, or historical version.

`resume_compatibility` is an optional closed list, not a release-wide Boolean. Each entry binds the exact `task_contract_id_at_admission` stored and recomputable under Section 21 as `source_task_contract_id` to one target-retained contract. The target release must still ship the listed adapter version and content digest, integration schema, route/router contract versions, and every listed runtime-entry byte digest. An absent entry, partial match, version-only match, or semantically claimed replacement is incompatible. A target runtime may use only the retained content it packages and verifies itself; the edge never authorizes execution of an arbitrary source-release wheel or mutation of the task to a newer contract.
...
That lifecycle migration covers only the working copy executing `upgrade`; ignored local state is neither committed nor propagated to other clones. A registered clone that later pulls the new Manifest uses the independent `workspace migrate` protocol in Section 14.12. The current committed launcher/runtime must contain a verified compatibility edge from that clone's recorded source local-state release and contract to the pulled target Manifest contract; absent support fails closed rather than treating the clone as fresh or rewriting empty state. Before any clone-local migration begins, the runtime-state gate scans every task transaction and non-archived integration state visible in that checkout. v0.1 blocks on every unfinished task transaction and requires recovery under the project checkout/release matching that journal before the pull-state migration can proceed. A non-archived task without an unfinished transaction permits migration only when the exact edge contains a matching `resume_compatibility` entry and the target release verifies all retained contract content. `workspace migrate` never rewrites or migrates a task.
...
Planning and dry-run do not create the local state. Before it exists, a first-init saved plan contains candidate lineage and workspace UUIDs, the candidate local-state contract derived from release/schema inputs, a workspace candidate bound to that contract, an empty replay-ledger candidate, and a digest of the normalized target path and requires Manifest and both local-state files to be absent. The workspace candidate and candidate Manifest independently render the same `plan_core` contract object; neither derives it from the other's bytes. The approved init transaction applies `workspace.json` and the empty ledger as one recoverable candidate pair before Manifest commit; the Manifest commit makes the pair authoritative, and pre-commit rollback removes both only under their original-absence and candidate-byte CAS. Every later saved plan binds both `project_id` and the persisted `workspace_instance_id`; applying it in another clone fails even when repository content and Manifest digests match.

The Manifest's `local_state_contract.contract_digest` is SHA-256 over RFC 8785 JCS of the contract object excluding `contract_digest`. It records the release and schema versions required for workspace state, approval replay, and task outbox items. `workspace.json` records the last contract actually applied in that clone. These fields do not make ignored local files pack-managed; they establish the version boundary that registration, the Task-state Service, `doctor`, and workspace migrations enforce. `last_transaction_binding_digest` copies the committing transaction's immutable journal binding so post-commit cleanup can be recognized even if the mutable journal was not advanced or was already removed.

When a registered clone receives a committed Manifest whose target local-state contract differs from `workspace.json`'s source contract, the mismatch is not corruption and is not eligible for `workspace register`. Contract identities are not ordinal. If the verified target release contains the exact directed source-to-target edge, the pinned CLI reports `AWP_WORKSPACE_MIGRATION_REQUIRED`; if verified metadata establishes only the reverse target-to-source edge, it reports `AWP_WORKSPACE_CONTRACT_AHEAD`; if neither exact direction is reachable, it reports `AWP_WORKSPACE_CONTRACT_DIVERGED`. “Ahead” is therefore a verified graph relationship, not a comparison of version strings. Migration-required state permits read-only diagnostics, `workspace migrate`, and only recovery already authorized by the current runtime allowlist. Ahead or diverged state permits read-only diagnostics and independently authorized recovery but not `workspace migrate`. All routing, task mutation, provider execution, and Reconciler-backed lifecycle writes remain blocked until the contracts match. An old task journal whose pinned runtime is no longer the committed or lifecycle-candidate runtime is reported as a recovery blocker; project-local target state cannot authorize execution of that source runtime.
...
Ordinary `doctor` is strictly read-only. It checks schemas, digests, cache, external runtimes, capabilities, ownership, drift, routing graph, workspace-contract alignment, unfinished transactions, active-task compatibility, static mount facts, and any previously recorded filesystem-probe evidence. For a differing registered local contract it reports `AWP_WORKSPACE_MIGRATION_REQUIRED`, `AWP_WORKSPACE_CONTRACT_AHEAD`, or `AWP_WORKSPACE_CONTRACT_DIVERGED` from the verified directed-edge relationship, including the exact edge or missing-edge reason. It never creates locks, refreshes evidence, intentionally updates target or cache state, or treats partial transaction state as success. A filesystem property that cannot be established from still-valid evidence is reported as `unverified`, not guessed as passing.
...
A fresh clone contains the repository-lineage Manifest and managed launcher but not ignored local workspace state. `.agent-workflow/bin/agent-stack workspace register` validates the runtime binding, Manifest, and managed ignore marker, requires both `workspace.json` and `approval-replay.json` to be absent, and acquires the bootstrap and project locks in that order. It creates a recoverable registration journal, writes a workspace candidate bound to the current Manifest's local-state release, contract digest, and schema versions first, and atomically renames the empty replay-ledger candidate last; that final rename is the registration commit point. Before commit, recovery may remove only matching candidate files under original-absence CAS. After commit, both files must validate as a pair bound to the same project, workspace, and local contract. The command does not change the Manifest, workflow lock, artifacts, or tasks and refuses during maintenance or an unrelated unfinished transaction. Route issuance, task commands, and Reconciler-backed writes block until registration succeeds; a partial registration requires registration recovery, while read-only diagnostics remain available and report the required action. Once registration or first init commits, a missing, malformed, identity-mismatched, or unsupported replay ledger fails closed and is never interpreted as an empty first-use ledger.

### 14.12 `workspace migrate`

An already registered working copy whose source local-state release and contract differ from the committed target Manifest may use `.agent-workflow/bin/agent-stack workspace migrate` only when the current verified release contains the exact directed source-to-target compatibility edge. A reverse-only relationship returns `AWP_WORKSPACE_CONTRACT_AHEAD`; an unreachable or ambiguous relationship returns `AWP_WORKSPACE_CONTRACT_DIVERGED`. The command acquires the project Reconciler lock followed by the exclusive runtime-state gate, revalidates both contract identities and the edge, and scans every task transaction plus every non-archived integration state reachable from the locked Trellis roots before it creates a workspace-migration journal.

Any unfinished task transaction blocks v0.1 workspace migration with `AWP_WORKSPACE_TASK_RECOVERY_BLOCK`, even when an edge otherwise advertises resume compatibility. The diagnostic identifies the journal's pinned Release Identity and instructs the user to restore or use the corresponding old project release/checkout and complete `task recover` there before retrying the pull-state migration. The pulled target Manifest, launcher, or compatibility edge cannot place that old runtime into the Section 20 allowlist. For each non-archived task without an unfinished transaction, the command computes its exact `source_task_contract_id` and requires a matching edge `resume_compatibility` entry whose adapter, integration schema, route/router contract, and runtime-entry content all exist with the declared digests in the target release. A missing or partial match returns `AWP_WORKSPACE_RESUME_INCOMPATIBLE`. A passing task remains byte-for-byte unchanged and later resumes its pinned mode through the target release's retained content; `workspace migrate` never edits, upgrades, archives, or reclassifies task state.

Only after that post-pull task gate passes does the command write a workspace-migration journal under `.agent-workflow/local/workspace-transactions/<transaction-id>.json`, enumerate the exact workspace, replay-ledger, and task-outbox preimages, and apply the edge-bound local migrations. It never modifies the Manifest, workflow lock, launcher, descriptor, managed artifacts, tasks, or source files. Task and journal visibility is checkout-local as defined in Section 17; success makes no claim about unsynchronized clones or branches.

The migration writes replay-ledger and outbox candidates first and atomically replaces `workspace.json` last. That final workspace rename is the local migration commit point because it records the newly applied release and aggregate contract. Before commit, recovery may resume or CAS-restore every exact preimage; after commit, recovery may finish cleanup only. An unfinished workspace-migration transaction must be recovered before a new migration begins; an unfinished task transaction remains the unconditional v0.1 blocker above. `recover --workspace-migration <id> --resume|--rollback` is the only workspace-migration recovery entry and follows the same lock order. A missing edge, unrecognized source schema, unexpected local path, partial candidate set, failed task-resume check, or failed CAS blocks without resetting local state.
...
`plan_core` contains all normalized operation inputs, identities, authorities, preconditions, non-Manifest candidate file states, the candidate local-state contract derived from release/schema inputs, provider approvals, active-task evidence, prospective transaction ID, and recovery runtime. It excludes `plan_core_digest`, `journal_binding_digest`, candidate Manifest bytes/digest, `plan_digest`, and diagnostics. The immutable header contains the fields defined in Section 16 but uses `plan_core_digest`, never final `plan_digest`. The plan envelope contains `plan_core`, `plan_core_digest`, the immutable header, `journal_binding_digest`, `candidate_manifest_digest`, and the candidate Manifest file-state record. Candidate Manifest bytes are reconstructable from the locked inputs and must match both their file-state record and `candidate_manifest_digest`; any workspace candidate must independently render the identical `plan_core` local-state contract rather than reading candidate Manifest bytes. Serialization may flatten schema fields, but the digest domains and exclusions are normative.
...
6. CAS-clean all probe paths, persist the successful evidence, advance the journal, then create a maintenance marker bound to the transaction ID and immutable `journal_binding_digest`. Rescan active tasks and only then begin apply.
...
For first init, `project_id` and `workspace_instance_id` are the approved candidate identities and `baseline_manifest_digest` is the schema-defined canonical null value. The immutable header is written before maintenance and never changes. The journal also records the final `plan_digest`, `candidate_manifest_digest`, and candidate Manifest as immutable sibling fields, but none participates in `journal_binding_digest`; they are verified through the Section 15 dependency DAG. Mutable journal fields such as `phase`, applied-file lists, diagnostics, retries, and rollback progress are likewise excluded. Every journal rewrite must preserve the header and all immutable sibling fields byte-for-byte and revalidate their respective digests.
...
After acquiring the applicable writer locks, a write transaction also acquires the exclusive runtime-state gate lock, atomically persists the transaction journal for process-crash recovery, creates `.agent-workflow/maintenance.json`, and then scans active tasks again before apply. It holds the gate through maintenance cleanup. The marker contains the transaction ID, `journal_binding_digest`, final `plan_digest`, and candidate Manifest generation. The binding validates only against the immutable journal header defined in Section 16, while `plan_digest` must match the journal's immutable sibling and Section 15 envelope; journal phase and other mutable fields may advance without rewriting the marker. Generated platform adapters, runtime loaders, and the heavy router must check this marker.
...
A fresh clone receives the committed launcher, descriptor, Manifest, and workflow lock even though local state is absent. After release verification, the pinned CLI permits `workspace register` and read-only diagnostics before local registration. A registered clone with a verified source-to-target local-contract edge permits read-only diagnostics, `workspace migrate`, and only recovery already authorized by the runtime allowlist. Contract-ahead or contract-diverged clones permit read-only diagnostics and independently authorized recovery but not migration. Other runtime and write commands remain blocked in all mismatch states.
...
workflow_contract:
  version: 1
  task_contract_id_at_admission: sha256-source-task-contract
  ...
  adapter_id: codex
  adapter_version_at_admission: 1.0.0
  adapter_content_digest_at_admission: sha256-value
  route_contract_version: 1
  runtime_entry_digests_at_admission:
    trellis-native: sha256-value
...
`task_contract_id_at_admission` is SHA-256 over RFC 8785 JCS of the immutable task-contract projection: the complete `workflow_contract` excluding that ID, the integration schema version, selected mode, and immutable mode-specific contract fields such as the heavy router contract version. It excludes lifecycle status, phase, claims, mutable artifact pointers, and transition history. `runtime_entry_digests_at_admission` is the exact non-empty map required by the selected mode, and the adapter content digest binds the projected adapter bytes rather than only its semantic version. Admission recomputes the ID from the locked content; resume and `resume_compatibility` matching use the stored fields and recomputed ID without consulting an executable source-release runtime.
...
- stable machine error codes such as `AWP_OWNERSHIP_DRIFT`, `AWP_WORKSPACE_MIGRATION_REQUIRED`, `AWP_WORKSPACE_CONTRACT_AHEAD`, `AWP_WORKSPACE_CONTRACT_DIVERGED`, `AWP_WORKSPACE_TASK_RECOVERY_BLOCK`, and `AWP_WORKSPACE_RESUME_INCOMPATIBLE`;
...
clone B: pull clone A commit
  ...
  -> active task without exact resume_compatibility fixture blocks migration
  -> exact resume-compatible pinned task fixture remains byte-identical and passes the task gate
  ...
  -> compatible task resumes through R1-retained adapter/router/runtime-entry content
...
clone C: pull a target with no source-to-target edge
  ...
  -> both remain read-only and do not offer workspace migrate
```
...
- a previously registered second clone can pull a differing committed Manifest, classify the exact directed source/target contract relationship, block migration on every unfinished task transaction, require explicit compatible resume entries for every non-archived task, complete or recover `workspace migrate`, and resume without modifying project-scoped or task authorities;
...
- **AC-48:** Every checkout-visible non-archived task without an unfinished transaction permits workspace migration only through an exact edge `resume_compatibility` entry retaining its integration schema, adapter, route/router contracts, and runtime-entry content digests. Missing or partial compatibility returns `AWP_WORKSPACE_RESUME_INCOMPATIBLE`; a passing migration leaves all task bytes unchanged and resumes through target-packaged retained content.
...
- **AC-50:** Wheel verification responsibility is split at process startup: uv/uvx verifies the actual downloaded wheel-container SHA-256 before CLI execution, while the CLI verifies only packaged identity and bundle claims plus their detached-manifest agreement. No release gate requires a running CLI to recover, size, or hash its original wheel container without a separately specified attestation channel.
- **AC-51:** Workspace contract mismatch uses exact directed graph relationships, never version-string ordering: a source-to-target edge reports `AWP_WORKSPACE_MIGRATION_REQUIRED`, a verified reverse-only relationship reports `AWP_WORKSPACE_CONTRACT_AHEAD`, and an unreachable or ambiguous pair reports `AWP_WORKSPACE_CONTRACT_DIVERGED` with migration unavailable.
...
4. **Runtime launcher and Task-state Service** — single-file launcher cold bootstrap, split pre-start container/post-start package verification, two-stage caller context, post-wheel descriptor validation, pinned runtime delivery, atomic workspace/ledger registration, post-pull task-gated cross-clone workspace migration, versioned replay ledger and outbox, integration union, task locks and CAS, claims, admitting/archive transactions, recovery, and maintenance coordination.
...
The seventh-version architecture remains stable, but this candidate is still `Changes required` until the post-pull task gate, trusted provider-broker handshake, and wheel-verification responsibility errata receive focused reviewer approval. Implementation planning and feature-spec decomposition remain prohibited until that confirmation.
