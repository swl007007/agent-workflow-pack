...
- Transparent resume of a non-archived task after its checkout pulls a different local-state release contract. v0.1 requires recovery, completion, and archive under the source release before workspace migration; multi-version retained-runtime resume is deferred to v0.2 or later.
...
`trellis_task_layout_digest = SHA256(JCS(normalized_trellis_task_layout))`. The normalized object is the closed field set shown above after metadata declarations are schema-normalized and set-semantic exact-path entries are sorted; it contains no diagnostics or discovered task paths. `runtime_namespace` is a locked Trellis-owned repository-relative root. `active_root` and `archive_root` must be strict descendants of it, must resolve as real non-symlink directories when present, and may not escape into another project namespace. The Resolver requires the archive root to be explicitly partitioned from active-task enumeration even when nested, derives protected globs for both roots, and includes the complete layout and its digest in the adapter and artifact-bundle digests. A new task ref must be below `active_root` but outside `archive_root`.

First init, workspace registration, lifecycle local-state migration, and `workspace migrate` record the exact current layout digest plus the literal runtime, active, and archive roots in `workspace.json`. After a pull changes the committed release, the target CLI must statically reverify that recorded source layout against the source release metadata described in Section 8; local bytes alone cannot redefine the scan roots. Before workspace migration, the runtime-state gate scans the normalized union of source and target active/archive roots. A root collision, unsafe nesting change, case/Unicode alias, missing source declaration, or partition that cannot distinguish active from archived tasks returns `AWP_WORKSPACE_TASK_LAYOUT_AMBIGUOUS` and blocks.
...
Ordinary `doctor` is strictly read-only. It checks schemas, digests, cache, external runtimes, capabilities, ownership, drift, routing graph, workspace-contract alignment, unfinished transactions, active-task compatibility, source/target task-layout evidence, static mount facts, and any previously recorded filesystem-probe evidence. For a differing registered local contract it reports `AWP_WORKSPACE_MIGRATION_REQUIRED`, `AWP_WORKSPACE_CONTRACT_AHEAD`, or `AWP_WORKSPACE_CONTRACT_DIVERGED` only from fully verified directed-edge metadata. When the source detached manifest, compatibility bundle, integration schema, or task layout is absent from the verified cache, it reports `AWP_WORKSPACE_SOURCE_METADATA_REQUIRED` and does not fetch or infer. It never creates locks, refreshes evidence, intentionally updates target or cache state, or treats partial transaction state as success. A filesystem property that cannot be established from still-valid evidence is reported as `unverified`, not guessed as passing.
...
An already registered working copy whose source local-state release and contract differ from the committed target Manifest enters `.agent-workflow/bin/agent-stack workspace migrate` as the only write-capable inspection/migration command. Before acquiring target-project writer locks or creating a migration journal, it may acquire the release-cache lock and use the current packaged trust policy plus `workspace.json`'s source release ID/version and manifest digest to fetch the exact immutable source detached manifest and hash-verified source distribution into the user cache. It parses only the schema-allowlisted static metadata described in Section 8 and executes no source code. An absent target-owned source-to-target edge returns `AWP_WORKSPACE_CONTRACT_AHEAD` only when the verified source compatibility bundle contains the exact reverse edge; two verified missing directions return `AWP_WORKSPACE_CONTRACT_DIVERGED`; incomplete source evidence returns `AWP_WORKSPACE_SOURCE_METADATA_REQUIRED`. None performs a target-project write.

For a valid source-to-target edge, the command acquires the project Reconciler lock followed by the exclusive runtime-state gate and revalidates both contract identities, the source manifest and bundle roots, the target edge, and the source layout snapshot. It normalizes the verified source and target Trellis layout declarations, scans the union of both active roots while excluding their respective archive partitions, scans both archive roots, and scans every task-transaction journal. A task found only through the source layout is parsed with the verified source integration schema; one found only through the target layout uses the target schema; a path visible through both must produce the same task identity and archived/non-archived result under both schemas. Any unsafe root relationship, conflicting interpretation, or source/target enumeration ambiguity returns `AWP_WORKSPACE_TASK_LAYOUT_AMBIGUOUS`.

Any unfinished task transaction blocks v0.1 workspace migration with `AWP_WORKSPACE_TASK_RECOVERY_BLOCK`. Any task directory under either active partition, or any archive-root task whose integration state is missing, invalid under the verified source/target schema, or not exactly `lifecycle.status: archived`, blocks with `AWP_WORKSPACE_ACTIVE_TASK_BLOCK`. Diagnostics identify the source release and instruct the user to restore that project checkout, run the matching task recovery when necessary, complete the task, and archive it before pulling/retrying the target migration. The pulled target Manifest, launcher, or compatibility edge cannot authorize the source runtime. v0.1 has no retained catalog or loader exception: `workspace migrate` never edits, resumes, upgrades, archives, or reclassifies task state.

Only after the task scan proves that no unfinished transaction or non-archived task exists does the command write a workspace-migration journal under `.agent-workflow/local/workspace-transactions/<transaction-id>.json`, enumerate the exact workspace, replay-ledger, and task-outbox preimages, and apply the edge-bound local migrations. It never modifies the Manifest, workflow lock, launcher, descriptor, managed artifacts, tasks, or source files. Task and journal visibility is checkout-local as defined in Section 17; success makes no claim about unsynchronized clones or branches.

The migration writes replay-ledger and outbox candidates first and atomically replaces `workspace.json` last. That final workspace rename is the local migration commit point because it records the newly applied release, aggregate contract, detached-manifest digest, and target Trellis-layout snapshot. Before commit, recovery may resume or CAS-restore every exact preimage; after commit, recovery may finish cleanup only. An unfinished workspace-migration transaction must be recovered before a new migration begins; unfinished task transactions and non-archived tasks remain the unconditional v0.1 blockers above. `recover --workspace-migration <id> --resume|--rollback` is the only workspace-migration recovery entry and follows the same lock order. A missing edge, unrecognized source schema, unexpected local path, partial candidate set, failed task/layout scan, or failed CAS blocks without resetting local state.
...
- stable machine error codes such as `AWP_OWNERSHIP_DRIFT`, `AWP_WORKSPACE_MIGRATION_REQUIRED`, `AWP_WORKSPACE_SOURCE_METADATA_REQUIRED`, `AWP_WORKSPACE_CONTRACT_AHEAD`, `AWP_WORKSPACE_CONTRACT_DIVERGED`, `AWP_WORKSPACE_TASK_RECOVERY_BLOCK`, `AWP_WORKSPACE_ACTIVE_TASK_BLOCK`, and `AWP_WORKSPACE_TASK_LAYOUT_AMBIGUOUS`;
...
clone B: pull clone A commit
  ...
  -> workspace migrate statically verifies the R0 manifest, compatibility bundle, integration schema, and task layout without executing R0 code
  ...
  -> changed or ambiguous R0/R1 task roots block rather than skipping source tasks
  -> restore R0 checkout, complete and archive every remaining task, then pull R1 again
...
clone C: pull a target with no source-to-target edge
  ...
  -> all three outcomes remain read-only and do not apply a workspace migration
```
...
- a previously registered second clone can pull a differing committed Manifest, statically verify the source release and source/target Trellis layouts without executing source code, block migration on every unfinished task transaction or non-archived task, complete or recover `workspace migrate` only after all visible tasks are archived, and leave project-scoped task authorities unchanged;
...
- **AC-48:** v0.1 has no retained-runtime resume branch. Every checkout-visible non-archived task blocks `workspace migrate` with `AWP_WORKSPACE_ACTIVE_TASK_BLOCK`, regardless of mode or apparent contract compatibility. The task must be recovered, completed, and archived under the source release; migration never rewrites or resumes it.
...
4. **Runtime launcher and Task-state Service** — single-file launcher cold bootstrap, split pre-start container/post-start package verification, two-stage caller context, post-wheel descriptor validation, pinned runtime delivery, atomic workspace/ledger registration, source-static-metadata verification and strict no-active-task cross-clone workspace migration, versioned replay ledger and outbox, integration union, task locks and CAS, claims, admitting/archive transactions, recovery, and maintenance coordination.
...
The ninth-version architecture remains stable, but this candidate is still `Changes required` until the strict no-active-task workspace migration, source-layout/static-metadata verification, and asset-size wording errata receive focused reviewer approval. Implementation planning and feature-spec decomposition remain prohibited until that confirmation.
