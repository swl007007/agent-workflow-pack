...
That lifecycle migration covers only the working copy executing `upgrade`; ignored local state is neither committed nor propagated to other clones. A registered clone that later pulls the new Manifest uses the independent `workspace migrate` protocol in Section 14.12. The current committed launcher/runtime must contain a verified compatibility edge from that clone's recorded source local-state release and contract to the pulled target Manifest contract; absent support fails closed rather than treating the clone as fresh or rewriting empty state. Both same-clone lifecycle upgrade and clone-local migration invoke the single Section 9 `scan_task_quiescence` contract over the verified source and target layouts and schemas. v0.1 blocks on every unfinished task transaction, every task not proven fully archived, and every nonempty layout state that the target would no longer recognize at the same path and semantic role. Recovery, completion, and archive occur only after restoring the project checkout/release matching the source task contract; neither lifecycle migration nor `workspace migrate` rewrites, resumes, or migrates Trellis task/layout state.

The source release is identified only by the validated `workspace.json` release ID, version, detached-manifest digest, and recorded Trellis-layout digest. The current trust policy derives the source immutable-release manifest locator from that version and repository identity, verifies the exact detached manifest and Release Identity, and then treats the hash-verified source wheel solely as a data archive. The static inspector applies the Section 23 download, path, count, size, collision, and compression protections and reads only schema-allowlisted member names for Release Identity, compatibility, the complete Trellis discovery layout, integration schemas, metadata parser/classifier schemas, and task-transaction journal schemas and phase tables. It recomputes their declared bundle roots and never imports, builds, or executes source-release code. Ordinary `doctor` uses only already verified cached source metadata and otherwise reports `AWP_WORKSPACE_SOURCE_METADATA_REQUIRED` as unverified. `workspace migrate` may fetch and cache that exact trusted source release before target-project mutation. A missing manifest, digest mismatch, unavailable immutable asset, unsupported static schema/parser/classifier, or bundle-root mismatch blocks without classifying the relationship as ahead or diverged.
...
A discovered task is a real non-symlink directory at that exact depth whose declared integration path is a regular non-symlink file within the byte limit and valid under one listed integration schema version. A grammar-matching directory with missing, oversized, malformed, or unsupported integration state blocks rather than disappearing from enumeration. Declared nested partition roots such as `archive_root` and entries in `allowed_non_task_entries` are the only exceptions to `unknown_root_entry_policy: block`; every other file, directory, symlink, excess-depth entry, or case/Unicode alias at a scanned root blocks. Missing declared roots are canonical empty state, but a present root of the wrong type blocks. Count and byte limits are hard bounds: exceeding them blocks and never truncates the scan.
...
`task_transaction_discovery` is likewise closed. An absent root is canonical empty; a present root must be a real non-symlink directory and has no subdirectories. Unknown names or entries block. The journal schema and phase classifier define the complete operation/phase table and normalized task-ref projection. A schema-valid journal is unfinished whenever its phase is not one of the declared terminal phases. An unsupported schema or classifier, corrupt journal, illegal phase transition, oversized file, unknown entry, or count overflow blocks conservatively because terminal state cannot be proven.

First init, workspace registration, lifecycle local-state migration, and `workspace migrate` record the exact normalized layout snapshot and digest in `workspace.json`. After a pull changes the committed release, the target CLI must statically reverify that recorded source layout and every named integration, metadata, journal, parser, classifier, and phase-table schema against the source release metadata described in Section 8; local bytes alone cannot redefine the scan or interpretation contract. A root collision, unsafe nesting change, case/Unicode alias, missing declaration, unsupported static interpreter, or partition that cannot distinguish active from archived tasks returns `AWP_WORKSPACE_TASK_LAYOUT_AMBIGUOUS` and blocks.
...
scan_task_quiescence(
  ...
  source_schemas,
  target_schemas
) -> canonical task_quiescence_snapshot + blockers

task_quiescence_digest = SHA256(JCS(task_quiescence_snapshot))
```

The snapshot has digest domain `agent-workflow.task-quiescence.v1` and records the source and target layout digests and schema-bundle digests; every normalized task path and its source/target active-or-archive discovery roles; integration byte hash, normalized mode, schema ID/version, lifecycle status, state revision, and pinned task-contract identity; every metadata path, byte hash, mode, parser/classifier identity, parsed task references, semantic role, and empty/nonempty classification; and every task-transaction path, byte hash, mode, schema, operation, phase, task ref, and terminal classification. It also records sorted unknown-entry, collision, and limit findings, which must all be empty for a successful result. All sets are sorted by normalized repository-relative path and stable IDs before JCS.

The scanner enumerates the union of source and target active/archive partitions, metadata contracts, and task-transaction roots while applying each side's own verified schemas. A source-only path is interpreted only by the verified source declarations; a target-only path only by the verified target declarations; an overlapping path is parsed independently under both. A path visible under both contracts must have the same task identity, active/archive role, lifecycle classification, metadata semantic role and task references, and journal operation, phase, task ref, and terminal result; conflicting interpretations return `AWP_WORKSPACE_TASK_LAYOUT_AMBIGUOUS`. A valid unfinished journal returns `AWP_WORKSPACE_TASK_RECOVERY_BLOCK`; a non-archived task returns `AWP_WORKSPACE_ACTIVE_TASK_BLOCK`.

When source and target layout digests differ, v0.1 applies an additional preservation rule. Every nonempty source task, archive, or metadata object must remain recognized at the same normalized path and semantic role by the target layout. Source-only task or archive state must be absent; source-only metadata must be absent under a contract that permits absence or classifier-proven canonical empty. Target-only task/archive state must be absent, and target-only metadata must likewise be permitted-absent or canonical empty. An archived task under a source-only archive root therefore blocks. Conflicting dual-schema interpretation is ambiguous; otherwise any nonempty state that would be lost from the target discovery surface returns `AWP_WORKSPACE_LAYOUT_STATE_STRANDED`. The scanner never moves, rewrites, deletes, archives, reindexes, or reclassifies Trellis state. True task-layout migration is outside v0.1.
...
When a registered clone receives a committed Manifest whose target local-state contract differs from `workspace.json`'s source contract, the mismatch is not corruption and is not eligible for `workspace register`. Contract identities are not ordinal. If the verified target release contains the exact directed source-to-target edge, the relationship is `migration-required`. If that edge is absent, `ahead` is legal only after the current trust policy uses the recorded source release ID/version and detached-manifest digest to statically verify the source manifest and compatibility bundle, without executing source code, and finds the exact reverse target-to-source edge. If both verified directions are absent, the relationship is `diverged`; if required source metadata is unavailable or unverifiable before that determination, the relationship remains `unknown` rather than being guessed.

The launcher, `doctor`, and `workspace migrate` derive human and JSON output from one structured workspace-state diagnostic:

```yaml
schema_version: 1
relationship: matching | migration-required | ahead | diverged | unknown
source_evidence: verified | missing | invalid | not-required
task_quiescence: not-evaluated | quiescent | blocked | ambiguous
primary_blocker: AWP_WORKSPACE_SOURCE_METADATA_REQUIRED
```

The dimensions are independent, and `primary_blocker` is null only when the requested command has no blocking condition. A target-owned source-to-target edge may establish `relationship: migration-required` while source static evidence is still `missing`; the primary blocker is then `AWP_WORKSPACE_SOURCE_METADATA_REQUIRED`, not migration execution. `ahead` may be reported only with `source_evidence: verified`; missing or invalid evidence is never reported as diverged. Task quiescence remains `not-evaluated` until the exact source evidence required by the Section 9 scanner is verified. After verification, successful scanning sets `quiescent`; unsupported schemas, conflicting interpretations, unknown entries, or unsafe partitions set `ambiguous`; valid task/layout blockers set `blocked`.

Primary-blocker selection is deterministic across all three commands: missing or invalid required source evidence precedes relationship-only blockers; then `AWP_WORKSPACE_CONTRACT_AHEAD` or `AWP_WORKSPACE_CONTRACT_DIVERGED`; then `AWP_WORKSPACE_TASK_LAYOUT_AMBIGUOUS`; then `AWP_WORKSPACE_TASK_RECOVERY_BLOCK`; then `AWP_WORKSPACE_ACTIVE_TASK_BLOCK`; then `AWP_WORKSPACE_LAYOUT_STATE_STRANDED`; and finally a commit-time `AWP_TASK_QUIESCENCE_CHANGED`. `AWP_WORKSPACE_MIGRATION_REQUIRED` is the primary state only when the relationship is migration-required, source evidence is verified, and task quiescence has not exposed a more specific blocker. Migration-required state permits read-only diagnostics, `workspace migrate`, and only recovery already authorized by the current runtime allowlist. Ahead or diverged state permits read-only diagnostics and independently authorized recovery but not migration. All routing, task mutation, provider execution, and Reconciler-backed lifecycle writes remain blocked until the contracts match. An old task journal whose pinned runtime is no longer the committed or lifecycle-candidate runtime is reported as a recovery blocker; project-local target state cannot authorize execution of that source runtime.
...
Ordinary `doctor` is strictly read-only. It checks schemas, digests, cache, external runtimes, capabilities, ownership, drift, routing graph, workspace-contract alignment, unfinished transactions, active-task compatibility, source/target task-discovery evidence, static mount facts, and any previously recorded filesystem-probe evidence. For a differing registered local contract it emits the Section 12 structured relationship/source-evidence/task-quiescence diagnostic and uses the shared primary-blocker priority. When the source detached manifest, compatibility bundle, complete Trellis layout, integration schema, metadata parser/classifier, or task-journal schema/phase table is absent from the verified cache, it records `source_evidence: missing`, reports `AWP_WORKSPACE_SOURCE_METADATA_REQUIRED`, and does not fetch or infer. It never creates locks, refreshes evidence, intentionally updates target or cache state, or treats partial transaction state as success. A filesystem property that cannot be established from still-valid evidence is reported as `unverified`, not guessed as passing.
...
`plan_core` contains all normalized operation inputs, identities, authorities, preconditions, non-Manifest candidate file states, the candidate local-state contract derived from release/schema inputs, provider approvals, the applicable Section 9 `task_quiescence_snapshot` and `task_quiescence_digest`, prospective transaction ID, and recovery runtime. It excludes `plan_core_digest`, `journal_binding_digest`, candidate Manifest bytes/digest, `plan_digest`, and diagnostics. The immutable header contains the fields defined in Section 16 but uses `plan_core_digest`, never final `plan_digest`, and copies the task-quiescence digest as an explicit sibling binding. The plan envelope contains `plan_core`, `plan_core_digest`, the immutable header, `journal_binding_digest`, `candidate_manifest_digest`, and the candidate Manifest file-state record. Candidate Manifest bytes are reconstructable from the locked inputs and must match both their file-state record and `candidate_manifest_digest`; any workspace candidate must independently render the identical `plan_core` local-state contract rather than reading candidate Manifest bytes. Serialization may flatten schema fields, but the digest domains and exclusions are normative.
...
- the source/target layout and schema-bundle identities, canonical task-quiescence snapshot, and its domain-separated digest;
...
- the shared task-quiescence and active-task gate.
...
6. CAS-clean all probe paths, persist the successful evidence, advance the journal, then create a maintenance marker bound to the transaction ID and immutable `journal_binding_digest`. Rerun the Section 9 scanner and require the approved task-quiescence snapshot and digest to match before apply.
...
Planning records the canonical snapshot and digest in `plan_core`. After the Reconciler/runtime-state locks and maintenance marker are established, the scanner runs again and must exactly match the approved evidence before apply. Section 16 requires the same equality immediately before Manifest commit. Layout or schema change does not permit a second scanner, current-only fallback, or adapter-specific shortcut.

The shared result covers every non-archived integration state, metadata pointer/index interpretation, layout-preservation finding, and unfinished task transaction; it does not rely only on the current session pointer:

- `admitting`, `active`, `blocked`, `completed`, and `archiving` all remain gating states; only `archived` is non-gating.
...
- A layout-changing upgrade blocks with `AWP_WORKSPACE_LAYOUT_STATE_STRANDED` when source-only archived or metadata state would cease to be recognized, or when target-only task state already exists; v0.1 does not migrate that state.
- Any difference between the approved/journal-bound snapshot and either locked rescan blocks with `AWP_TASK_QUIESCENCE_CHANGED` while the lifecycle transaction remains pre-commit recoverable.
...
|---|---|---|---|
| `classify-only` | Any admitted route | No intent, task-ref, challenge, or approval fields | Read-only result; never accepted by an execution loader or `task admit` |
...
| `create-integrated-task` | `trellis-native` or `speckit-superpowers` only | Intent, task ref, absence precondition, fresh challenge, and `task_creation_approval: required` | May be consumed only by `task admit` with a matching approval proof |
...
`task_state_digest` covers the canonical non-archived-task scan, active pointers, modes, lifecycle revisions, and pinned contract digests. The integrated branch also covers the requested task-ref absence precondition. It is null only for a classification-only project with no task-state inputs. Staging under `.agent-workflow/local/` does not alter this digest. Any intervening admission, transition, archive, pointer change, or relevant ref creation makes an executable Decision stale.
...
Integrated task-creation approval and implementation activation are separate gates. The enforced platform approval mechanism returns a one-time proof binding approval ID, approval challenge, Route Decision digest, task ref, intent digest, `operation: create-integrated-task`, workspace instance, actor, time, and verifier ID/version. It proves direct approval of that envelope, not completeness of signal extraction or exclusive calculator origin. `task admit` accepts no free-form override for those fields and records the verified proof in revision 1. The proof may be consumed only by one task-admission transaction; recovery may continue that same transaction but may not apply it to another ref. `classify-only` and `execute-light` never carry task-creation approval fields. Implementation activation belongs in the heavy branch after the task exists.
...
- stable machine error codes such as `AWP_OWNERSHIP_DRIFT`, `AWP_WORKSPACE_MIGRATION_REQUIRED`, `AWP_WORKSPACE_SOURCE_METADATA_REQUIRED`, `AWP_WORKSPACE_CONTRACT_AHEAD`, `AWP_WORKSPACE_CONTRACT_DIVERGED`, `AWP_WORKSPACE_TASK_RECOVERY_BLOCK`, `AWP_WORKSPACE_ACTIVE_TASK_BLOCK`, `AWP_WORKSPACE_TASK_LAYOUT_AMBIGUOUS`, `AWP_WORKSPACE_LAYOUT_STATE_STRANDED`, and `AWP_TASK_QUIESCENCE_CHANGED`;
...
Test duplicate YAML keys, inheritance cycles, unknown fields, JCS digests, set sorting, Merkle inputs, path normalization, POSIX-mode normalization, marker parsing, Release Identity, detached-manifest trust rules, release-compatibility edges, source static-metadata schemas, the closed Trellis task-discovery/layout union, segment and filename grammars, integration/metadata/journal parser-classifier identities, task-quiescence snapshots and digests, the structured relationship/source-evidence/quiescence diagnostic, the saved-plan operation union, the complete acyclic `plan_core_digest -> journal_binding_digest -> candidate_manifest_digest -> plan_digest` graph, closed Route Decision branches, task/provider approval-verifier unions, caller-context envelopes, workspace-registration and workspace-migration journals, workspace/replay/outbox schema migrations, replay state transitions, provider-attempt journals and release receipts, and Manifest generations. A dependency-graph test rejects every digest-domain cycle, including a final `plan_digest` entering `journal_binding_digest` or candidate Manifest input and a workspace candidate deriving its contract from candidate Manifest bytes instead of the shared `plan_core` contract. Property/fuzz tests target path normalization, bounded metadata expansion, task-segment grammar, maximum depth/count/byte bounds, unknown-root entries, source/target task-root union and partitioning, archive extraction, release-manifest URLs and redirects, general URL handling, caller-context field normalization, and marker parsers.

### 26.2 Resolver and Policy Tests

Test dependencies, conflicts, stable IDs, capability ordering, route rules, source-of-truth separation, Trellis runtime-namespace constraints, closed discovery/parser/classifier references, and metadata-path cross-ownership rejection against every protected or artifact-managed path class.
...
clone C: pull a target with no source-to-target edge
  ...
  -> unavailable or unverifiable source metadata reports AWP_WORKSPACE_SOURCE_METADATA_REQUIRED
  -> launcher, doctor, and migrate emit identical relationship/source-evidence/quiescence dimensions and primary blocker
...
- **AC-51:** Workspace contract mismatch uses exact directed graph relationships, never version-string ordering. A source-to-target edge reports `AWP_WORKSPACE_MIGRATION_REQUIRED`; a reverse-only result reports `AWP_WORKSPACE_CONTRACT_AHEAD` only after the current trust policy statically verifies the recorded source immutable release and compatibility bundle without executing source code; two verified missing directions report `AWP_WORKSPACE_CONTRACT_DIVERGED`; incomplete source evidence reports `AWP_WORKSPACE_SOURCE_METADATA_REQUIRED` instead of guessing.
...
- **AC-55:** Same-clone lifecycle upgrade and post-pull `workspace migrate` invoke the same `scan_task_quiescence` implementation over verified source/target layouts and schemas. The scanner covers both active/archive partitions, metadata/index/pointer objects, and task journals; current-only roots or command-specific fallback parsers cannot satisfy either gate.
- **AC-56:** The canonical task-quiescence snapshot and digest are bound into the lifecycle plan and immutable journal header and into the workspace-migration journal before local writes. The exact scanner reruns after lock/maintenance acquisition and immediately before Manifest or `workspace.json` commit; any intervening task, integration, metadata, pointer/index, or journal change returns `AWP_TASK_QUIESCENCE_CHANGED` and remains pre-commit CAS-recoverable.
- **AC-57:** Launcher, `doctor`, and `workspace migrate` emit one structured workspace-state diagnostic with independent `relationship`, `source_evidence`, and `task_quiescence` dimensions plus a deterministic `primary_blocker`. A known migration edge with missing source evidence remains `relationship: migration-required` but is primarily blocked by `AWP_WORKSPACE_SOURCE_METADATA_REQUIRED`; ahead/diverged are never guessed without verified source evidence.
...
4. **Runtime launcher and Task-state Service** — single-file launcher cold bootstrap, split pre-start container/post-start package verification, two-stage caller context, post-wheel descriptor validation, pinned runtime delivery, atomic workspace/ledger registration, source-static-metadata verification, the shared bounded Trellis quiescence scanner, strict no-active-task/no-stranded-state cross-clone workspace migration, versioned replay ledger and outbox, integration union, task locks and CAS, claims, admitting/archive transactions, recovery, and maintenance coordination.
...
The tenth-version architecture remains stable, but this candidate is still `Changes required` until the closed Trellis discovery contract, v0.1 layout-preservation rule, shared task-quiescence scanner, commit-time digest revalidation, and structured workspace diagnostic receive focused reviewer approval. Implementation planning and feature-spec decomposition remain prohibited until that confirmation.
