...
Version `0.1.3` is released only from the immutable GitHub release at
`swl007007/agent-workflow-pack`. Do not install from an untagged branch, a mutable asset,
or a locally supplied manifest.

Versions `0.1.0` through `0.1.2` are retained as failed publications and must not be
installed.
...
Run the canonical bootstrap command from the `v0.1.3` release, then enter the target
project and execute:
